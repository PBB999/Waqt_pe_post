...
import { getParcelByReceiverId, getParcelBySenderId } from '../redux/parcel/parcelActions';
...
const ToMe = ({ }) => {
    ...
    const receiverParcels = parcelState.receiverParcels;
    ...
    useEffect(() => {
        ...
        dispatch(getParcelByReceiverId(userInfo._id)); ToMe
    }, [userInfo])

    useEffect(() => {
        if (!receiverParcels) return;
        setDeliveries(receiverParcels.map((parcel, index) => (
            {
                id: parcel._id,
                name: index + 1,
    ...
    const renderDeliveryItem = ({ item }) => (
        <View style={styles.deliveryCard}>
            ...
            <TouchableOpacity
                ...
                onPress={() => navigation.navigate('SenderDeliveryDetails', { deliveryId: item.id })}
            >
